...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
            #positions of living ghosts with highest probability.
            livingGhostPos=i.argMax()
            for action in legal:
                #Successor position of a position after an action.
                nextMove=Actions.getSuccessor(pacmanPosition, action)
                #Maze distance between any two positions next move and livingGhostPos.
                distance=self.distancer.getDistance(nextMove,livingGhostPos)
                if(distance<temp):
                    temp=distance
                    bestMove=action
            #Best action to take is returned.
            return bestMove
        return None
